defmodule CQEx.Result do
  ...
  def convert(r) when Record.is_record(r, :cql_result) do
    %CQEx.Result{record: r}
  end

  def size(%CQEx.Result{record: rec}), do: _size rec
  def size(rec), do: _size rec

  def head(%CQEx.Result{record: rec}), do: _head rec
  def head(rec), do: _head rec

  def head(%CQEx.Result{record: rec}, Opts), do: _head rec, Opts
  def head(rec, Opts), do: _head rec, Opts

  def tail(%CQEx.Result{record: rec}), do: tail rec
  def tail(rec) do
    %CQEx.Result{record: _tail rec}
  end

  def next(%CQEx.Result{record: rec}), do: next rec
  def next(rec) do
    {head, tail} = _next rec
    {head, %CQEx.Result{record: tail}}
  end

  def all_rows(%CQEx.Result{record: rec}, Opts), do: _all_rows rec, Opts
  def all_rows(rec, Opts), do: _all_rows rec, Opts

  def all_rows(%CQEx.Result{record: rec}), do: _all_rows rec
  def all_rows(rec), do: _all_rows rec

  def has_more_pages(%CQEx.Result{record: rec}), do: _has_more_pages rec
  def has_more_pages(rec), do: _has_more_pages rec

  def fetch_more(%CQEx.Result{record: rec}), do: fetch_more rec
  def fetch_more(rec) do
    %CQEx.Result{record: _fetch_more rec}
  end

  def fetch_more_async(%CQEx.Result{record: rec}), do: _fetch_more_async rec
  def fetch_more_async(rec), do: _fetch_more_async rec
end

defimpl Enumerable, for: CQEx.Result do
